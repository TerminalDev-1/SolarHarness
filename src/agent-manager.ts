...
const MAX_SUB_WORKERS_PER_WORKER = 8;

type SubWorkerRequest = { tasks: AgentTask[] };
type SubWorkerControl = {
...
export class AgentManager {
  ...
  async spawn(input: SpawnSubAgentInput): Promise<AgentRecord> {
    ...
    if (input.parentId && !parent) throw new Error(`Unknown parent worker: ${input.parentId}`);
    if (parent?.depth === 1) throw new Error("Sub-workers cannot create another delegation level.");
    if (this.executingCount() >= MAX_CONCURRENT_AGENTS) {
      throw new Error(`Solar Harness allows no more than ${MAX_CONCURRENT_AGENTS} concurrent worker processes.`);
    }

    const id = `${parent ? "sub" : "worker"}-${randomUUID().slice(0, 6)}`;
    ...
    await this.runWorker(record);
    return record;
  }

  async orchestrate(input: OrchestrateInput, actorId: "coordinator" | string = "coordinator"): Promise<AgentRecord[]> {
    ...
    if (input.action === "cancel") {
      ...
      record.latestActivity = actorId === "coordinator" ? "Cancelled by Solar" : `Cancelled by ${this.records.get(actorId)?.name ?? actorId}`;
    }
  ...
  setReasoning(agentId: string, reasoning: ReasoningEffort, actorId: "coordinator" | string = "coordinator"): AgentRecord {
    const record = this.getRecord(agentId);
    if (!record) throw new Error(`Unknown worker: ${agentId}`);
    this.assertControl(actorId, record);
    if (record.reasoningPinned && actorId !== "coordinator" && reasoning !== "light") {
    ...
    record.latestActivity = actorId === "coordinator"
  ...
  private async runWorker(record: AgentRecord): Promise<void> {
    const controller = new AbortController();
    this.aborters.set(record.id, controller);
    record.status = "running";
    record.startedAt = new Date();
    record.latestActivity = record.depth === 0 ? "Starting worker session" : "Starting Light sub-worker session";
    try {
      const result = await this.provider.run(this.workerPrompt(record), {
        ...this.options,
        role: record.depth === 0 ? "worker" : "sub-worker",
      ...
      const request = record.depth === 0 ? extractSubWorkerRequest(result.text) : undefined;
      if (request) {
        record.status = "waiting";
        record.latestActivity = `Delegating ${request.tasks.length} sub-worker${request.tasks.length === 1 ? "" : "s"}`;
        const childPromises = request.tasks.slice(0, MAX_SUB_WORKERS_PER_WORKER).map(task => this.spawn({
          ...task,
        ...
        record.status = "running";
        record.latestActivity = "Reviewing sub-worker reports";
        let resumed = record.sessionId
          ? await this.provider.resume(record.sessionId, [
              "Your requested sub-workers have finished. You control their work product; inspect these reports, perform any necessary integration or validation yourself, and then report the complete outcome to Solar.",
              "If a direct sub-worker needs a follow-up, emit exactly one line instead of a final report: SOLAR_SUBWORKER_TOOL: {\"action\":\"inject_context|cancel|set_reasoning\",\"agentId\":\"name-or-id\",\"context\":\"required for inject_context\",\"reasoning\":\"light\"}. You may control only your children and cannot raise them above Light.",
              "Do not request another delegation level.",
              `Sub-worker reports:\n${reports}`
            ].join("\n\n"), { ...this.options, role: "worker", reasoning: record.reasoning, signal: controller.signal, onEvent: event => this.recordActivity(record, event) })
          : await this.provider.run([this.workerPrompt(record), `Sub-worker reports:\n${reports}`].join("\n\n"), { ...this.options, role: "worker", reasoning: record.reasoning, signal: controller.signal, onEvent: event => this.recordActivity(record, event) });
        let controlRounds = 0;
        let control = extractSubWorkerControl(resumed.text);
        while (control && controlRounds < 4) {
          ...
          try {
            if (control.action === "set_reasoning") {
              ...
              outcome = child ? `${child.name} is ${child.status}. Latest report:\n${child.report ?? child.error ?? child.latestActivity}` : "Sub-worker control completed.";
            }
          } catch (error) {
          ...
          resumed = await this.provider.resume(resumed.sessionId ?? record.sessionId!, [
            `Sub-worker control result:\n${outcome}`,
            "Continue integrating the work. You may emit another SOLAR_SUBWORKER_TOOL line if a direct child needs a follow-up; otherwise provide your final report to Solar."
          ].join("\n\n"), { ...this.options, role: "worker", reasoning: record.reasoning, signal: controller.signal, onEvent: event => this.recordActivity(record, event) });
          control = extractSubWorkerControl(resumed.text);
        }
      ...
      record.latestActivity = record.depth === 0 ? "Completed and reported to Solar" : `Completed and reported to ${this.records.get(record.parentId ?? "")?.name ?? "parent worker"}`;
    } catch (error) {
  ...
  private workerPrompt(record: AgentRecord): string {
    const role = record.depth === 0 ? "implementation worker" : "implementation sub-worker";
    const delegation = record.depth === 0
      ? [
          "You may delegate genuinely independent parts of your assignment to named sub-workers. They share your workspace and report back to you.",
          ...
          "Request no more than eight sub-workers. Do not use sub-workers for work you can efficiently complete yourself. All sub-workers begin pinned to Light reasoning; only Solar may authorize an increase."
        ]
      : [
          "You cannot delegate further. Work directly on the assignment from your parent worker.",
          "Your reasoning is pinned to Light unless Solar explicitly authorizes another level."
        ];
    return [
      `You are ${record.name}, a ${role} launched by Solar Harness. You are not the coordinator.`,
      "Coordinator-only constraints do not apply to you. Use your workspace tools to inspect, create, edit, and validate files inside the current workspace.",
      ...delegation,
  ...
  private async continueWorker(record: AgentRecord, context: string): Promise<void> {
    ...
    try {
      const result = await this.provider.resume(record.sessionId!, [
        `You are continuing as ${record.name}, an implementation ${record.depth === 0 ? "worker" : "sub-worker"}.`,
        `New context from Solar: ${context}`,
        "Apply this context to your previous task if action is needed, then send an updated report."
      ].join("\n\n"), { ...this.options, role: record.depth === 0 ? "worker" : "sub-worker", reasoning: record.reasoning, onEvent: event => this.recordActivity(record, event) });
      record.report = stripControlLines(result.text);
  ...
  private uniqueName(requested: string): string {
    const base = requested.trim().replace(/\s+/g, "-") || "Worker";
  ...
  private assertControl(actorId: "coordinator" | string, target: AgentRecord): void {
    if (actorId === "coordinator") return;
    if (target.parentId !== actorId) throw new Error("Workers may control only their own direct sub-workers.");
  }
...
function extractSubWorkerRequest(text: string): SubWorkerRequest | undefined {
  const match = text.match(/^SOLAR_SUBDELEGATE:\s*(\{[^\r\n]+\})\s*$/m);
  if (!match) return undefined;
  const parsed = JSON.parse(match[1]) as SubWorkerRequest;
  if (!Array.isArray(parsed.tasks) || parsed.tasks.length === 0 || parsed.tasks.length > MAX_SUB_WORKERS_PER_WORKER) {
    throw new Error("Worker requested an invalid sub-worker plan (expected one to eight tasks).");
  }
  for (const task of parsed.tasks) {
    if (!task.name?.trim() || !task.title?.trim() || !task.instructions?.trim()) {
      throw new Error("Every sub-worker request requires a name, title, and instructions.");
    }
  }
  return parsed;
}
...
function stripControlLines(text: string): string {
  return text
    ...
    .replace(/^SOLAR_SUBWORKER_TOOL:\s*\{[^\r\n]+\}\s*$/gm, "")
    .trim();
}

function extractSubWorkerControl(text: string): SubWorkerControl | undefined {
  const match = text.match(/^SOLAR_SUBWORKER_TOOL:\s*(\{[^\r\n]+\})\s*$/m);
  if (!match) return undefined;
  const parsed = JSON.parse(match[1]) as SubWorkerControl;
  if (!parsed.agentId || !["cancel", "inject_context", "set_reasoning"].includes(parsed.action)) {
    throw new Error("Worker emitted an invalid sub-worker control request.");
  }
